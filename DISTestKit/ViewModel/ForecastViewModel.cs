...
namespace DISTestKit.ViewModel
{
    public class ForecastViewModel : PageViewModel, INotifyPropertyChanged
    {
        ...
        {
            get => _selectedDate;
            set { _selectedDate = value; OnPropertyChanged(nameof(SelectedDate)); RefreshHistorical(); }
        }
        ...
        {
            get => _selectedTime;
            set { _selectedTime = value; OnPropertyChanged(nameof(SelectedTime)); RefreshHistorical(); }
        }
        ...
        private async Task StartRealtimeAsync(ObservableCollection<DateTimePoint> vals)
        {
            while (true)
            {
                ...
                var ts = DateTimeOffset.FromUnixTimeMilliseconds(dto.LastPduReceivedTimestampMs).LocalDateTime;
                App.Current.Dispatcher.Invoke(() =>
                {
                    vals.Add(new DateTimePoint(ts, dto.PdusInLastSixtySeconds));
                    if (vals.Count > 60) vals.RemoveAt(0);
